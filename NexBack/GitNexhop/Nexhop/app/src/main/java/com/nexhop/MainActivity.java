...
import com.nexhop.ui.Checkout;
import com.nexhop.ui.LeftNavAdapter;
import com.nexhop.ui.MainFragment;
import com.nexhop.ui.OnSale;
...
// TODO Auto-generated method stub
        ...
        if (!Constants.HOMEFRAGMENT_CHECK) // set to false if user navigating in app through navigation drawer otherwise true
        {
            if (Constants.FRAGMENT_MODE == 1) // user navigating to inner views (deep navigation) set to 1
            {
                Constants.FRAGMENT_MODE = 0;
                Constants.HOMEFRAGMENT_CHECK = true;
                setupContainer(1);
	...
	private void setupLeftNavDrawer()
	{
		...
		drawerLeft.setOnItemClickListener(new OnItemClickListener() {
					...
					long arg3)
			{
				...
				drawerLayout.closeDrawers();
                Constants.FRAGMENT_MODE = 1;
                Constants.HOMEFRAGMENT_CHECK = false;
                Constants.fRAGMENT_INNERVIEW_CHECK = false;
				setupContainer(pos);
			}
		});
	...
	private void setupContainer(int pos)
	{
		...
		{
			f = new com.nexhop.ui.Settings();
			title = "Settings";
		}
		else if (pos == 5)
		{
            SharedPreferences appPref= Utilities.getPrivatePreference(this);
            appPref.edit().clear().commit();
            SharedPreferences gcmPref= Utilities.getGCMPreferences(this);
